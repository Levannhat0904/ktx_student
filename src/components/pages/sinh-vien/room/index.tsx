...
const RoomDetailPage = () => {
  ...
  const { data: roomData, isLoading: isRoomLoading } = useQuery({
    queryKey: ["roomDetail"],
    queryFn: () => roomApi.getRoomDetail(6), // Replace with actual room ID from context/state
  });
  ...
  useEffect(() => {
    const fetchTimeline = async () => {
      if(roomData){
          ...
          const timelineResponse = await studentApi.getActivityLogs('room', roomData.room.id);
